...
resource "aws_instance" "dream_demo" {
  ...
  user_data = templatefile("user_data_old.sh", {
...
# The lifecycle rule ensures records are updated when instance IPs change
resource "aws_route53_record" "sprint_records" {
  ...
  type    = "A"
  ttl     = "300"
  records = [aws_instance.dream_demo[each.key].public_ip]
  
  # Always replace the record when the instance IP changes
  # This prevents conflicts when instances are recreated with new IPs
  lifecycle {
    replace_triggered_by = [aws_instance.dream_demo[each.key].public_ip]
  }
